...
export class ScoringSystem {
  ...
  public subscribeToScoreChanges(callback: (score: number, event: ScoreEvent) => void): () => void {
  ...
  public getStatistics() {
    return {
      ...
      averageScore: this.scoreHistory.length > 0 ? 
        this.currentScore / this.scoreHistory.length : 0,
      scoreBreakdown: this.getScoreBreakdown()
    };
  }

  private getScoreBreakdown() {
    return this.scoreHistory.reduce((breakdown, event) => {
...
export const useScore = ({ initialScore = 0, onScoreChange }: UseScoreOptions = {}) => {
